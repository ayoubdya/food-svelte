<script lang="ts">
  ...
  async function onFavourite(id: string) {
    if (!isFavourite) {
      await favourites.addFavouriteMeal(id);
      isFavourite = true;
    } else {
      favourites.removeFavouriteMeal(id);
      isFavourite = false;
    }
  }
</script>
    ...
    <div class="w-full max-w-7xl flex flex-col gap-3">
      <div class="flex flex-col lg:flex-row gap-3">
        <img
          src={meal.thumbnail}
          alt="meal"
          class="w-full aspect-square lg:w-1/2 lg:mb-auto object-cover rounded-3xl drop-shadow-xl"
        />
        <div class="flex flex-col gap-2">
          <div class="flex justify-between">
            <h1 class="text-3xl font-bold">{meal.name}</h1>
            <button on:click={() => onFavourite(meal.id)}>
              <svg
                viewBox="0 0 24 24"
                class={twMerge(
          ...
          <p>
            <span class="text-sm font-light">Category:</span>
            {meal.category}
          </p>
          ...
          {#if meal.tags.length > 0}
            <div class="flex gap-2 items-center">
              ...
              {#each meal.tags as tag}
                <span
                  class="text-sm font-light bg-gray-200 rounded-full px-2 py-1"
                >
                  {tag}
                </span>
              {/each}
