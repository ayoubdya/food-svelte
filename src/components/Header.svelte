...
<nav
  id="header"
  class="flex gap-4 items-center w-full h-20 p-3 justify-center border-b-[1px] mb-2"
>
  <a
    href="/"
    use:link
  ...
  >
  <div
    class="group flex items-center h-full w-full max-w-sm bg-gray-100 focus:bg-white rounded-2xl"
  >
    <svg
    ...
    >
      {#each ["name", "category", "area", "ingredient"] as searchType}
        <option value={searchType}
          >{searchType.charAt(0).toUpperCase() + searchType.slice(1)}</option
        >
      {/each}
    </select>
    <input
